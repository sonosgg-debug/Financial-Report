  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
        {/* Tabs */}
        <div className="flex space-x-1 bg-[#1e293b] p-1 rounded-xl border border-slate-800">
          <button
        ...
        <select 
          ...
          className="bg-[#1e293b] border border-slate-800 rounded-lg px-4 py-2 text-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 max-w-xs cursor-pointer"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-[#1e293b] rounded-2xl border border-slate-800 p-6 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-semibold text-slate-100">Asset Allocation</h2>
        ...
        <div className="bg-[#1e293b] rounded-2xl border border-slate-800 p-6 shadow-sm flex flex-col">
          <h2 className="text-xl font-semibold text-slate-100 mb-6">Current Holdings {activeTab !== 'ALL' && `(${activeTab})`}</h2>
                ...
                {filteredHoldings.sort((a,b) => b.currentValue - a.currentValue).map(holding => {
                  ...
                  <div key={`${holding.ticker}_${holding.account}`} className="flex items-center justify-between p-4 bg-[#0f172a] rounded-xl border border-slate-800/50 hover:border-slate-700/50 transition-colors">
                    ...
                    <div className="text-right">
                      <p className="font-bold text-white">{curSymbol}{holding.currentValue.toLocaleString(undefined, { minimumFractionDigits: isKrw ? 0 : 2, maximumFractionDigits: isKrw ? 0 : 2 })}</p>
                      <p className={`text-sm font-medium ${holding.unrealizedReturn >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
      ...
      <div className="bg-[#1e293b] rounded-2xl border border-slate-800 p-6 shadow-sm">
        <h2 className="text-xl font-semibold text-slate-100 mb-6">Daily Asset Value (KRW)</h2>
      ...
      <div className="bg-[#1e293b] rounded-2xl border border-slate-800 p-6 shadow-sm">
        <h2 className="text-xl font-semibold text-slate-100 mb-2">Daily Cumulative Return (%) - Time Weighted</h2>
        <p className="text-sm text-slate-400 mb-6">순수 펀드 운용 수익률 (입출금 왜곡 방지 적용)</p>
